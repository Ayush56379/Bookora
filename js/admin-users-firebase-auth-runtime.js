...
(() => {
  'use strict';
  if (window.__BOOKORA_ADMIN_AUTH_USERS_RUNTIME_V1__) return;
  window.__BOOKORA_ADMIN_AUTH_USERS_RUNTIME_V1__ = true;

  const CACHE_KEY = 'bookora_admin_auth_users_v1';
  const CACHE_TTL = 5 * 60 * 1000;
  const state = { users: [], loaded: false, fetching: false, rendering: false, route: false };
  ...
  const formatDate = value => {
    if (!value) return '—';
    try { const d = new Date(value); return Number.isNaN(d.getTime()) ? '—' : d.toLocaleString(); } catch (_) { return '—'; }
  };
  ...
  const loadCache = () => {
    try {
      ...
      if (!cached || !Array.isArray(cached.users) || !cached.users.length) return false;
  ...
  const render = () => {
    ...
    try {
      ...
      tbody.innerHTML = users.map(user => {
        ...
        const statusClass = status === 'active' ? 'status-active' : status === 'suspended' ? 'status-suspended' : status === 'pending' ? 'status-pending' : 'status-default';
        return `<tr class="admin-user-row" data-user-id="${esc(user.id)}">
          ...
          <td class="admin-user-cell">${master ? '<span class="master-badge">ADMIN</span>' : `<select class="admin-user-select user-role-select auth-user-role" data-id="${esc(user.id)}"><option value="buyer" ${role==='buyer'?'selected':''}>Buyer</option><option value="creator" ${role==='creator'?'selected':''}>Creator</option><option value="seller" ${role==='seller'?'selected':''}>Seller</option><option value="admin" ${role==='admin'?'selected':''}>Admin</option></select>`}</td>
          <td class="admin-user-cell">${master ? '<span class="admin-user-status status-active">ACTIVE</span>' : `<select class="admin-user-select user-status-select auth-user-status" data-id="${esc(user.id)}"><option value="active" ${status==='active'?'selected':''}>Active</option><option value="suspended" ${status==='suspended'?'selected':''}>Suspended</option><option value="pending" ${status==='pending'?'selected':''}>Pending</option></select>`}</td>
          ...
          <td class="admin-user-cell">${esc(formatDate(user.created_at || user.createdAt))}</td>
          <td class="admin-user-cell">${master ? '<span style="color:#64748b;font-size:12px">Protected</span>' : `<button type="button" class="admin-user-action user-save-btn auth-user-save" data-id="${esc(user.id)}">Save</button>`}</td>
        </tr>`;
      }).join('');
  ...
  const fetchUsers = async force => {
    ...
    if (!force && state.loaded && state.users.length) return;
    state.fetching = true;
    try {
      ...
      if (!list.length && Number(payload?.count || 0) === 0) {
        state.users = [];
      } else {
        state.users = list;
      }
      ...
      saveCache();
      render();
      window.dispatchEvent(new CustomEvent('bookora:admin-auth-users-loaded', { detail: { count: state.users.length } }));
    } catch (error) {
      console.warn('[Admin Auth Users] load failed:', error);
      if (!state.loaded) {
        const tbody = document.getElementById('admin-users-list');
        if (tbody) tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:50px;color:#b91c1c">Unable to load Firebase Authentication users. Please refresh after the backend finishes deploying.</td></tr>';
      }
  ...
  const saveUser = async button => {
    ...
    try {
      ...
      await window.firebase.firestore().collection('users').doc(String(user.id)).set({ role, status, updatedAt: window.firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
      user.role = role; user.status = status;
      saveCache(); render();
      window.dispatchEvent(new CustomEvent('bookora:admin-user-updated', { detail: { id, role, status } }));
    } catch (error) {
      console.error('[Admin Auth Users] save:', error);
  ...
  const bind = () => {
    ...
    if (search && !search.dataset.authUsersBound) {
      search.dataset.authUsersBound = '1';
      search.addEventListener('input', render);
    }
    const refresh = document.getElementById('admin-users-refresh');
    if (refresh && !refresh.dataset.authUsersBound) {
      refresh.dataset.authUsersBound = '1';
      refresh.addEventListener('click', () => fetchUsers(true));
    }
    render();
  };
