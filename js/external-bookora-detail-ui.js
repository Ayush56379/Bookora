...
async function openExternalPurchase(book) {
  ...
  if (!token) { window.location.hash = '#/login?returnTo=' + encodeURIComponent(window.location.hash || '#/explore'); return; }
  const response = await fetch(`${API_BASE_URL}/api/external/purchase/session`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ book_id: book.id }) });
...
function renderExternalPaymentStatus() {
  ...
  const isExternalStatus = hash.startsWith('#/external-payment');
  const successParams = hash.startsWith('#/payment/success') ? new URLSearchParams(hash.split('?')[1] || '') : null;
  const sessionId = (isExternalStatus ? new URLSearchParams(hash.split('?')[1] || '').get('session_id') : successParams?.get('external_session')) || '';
  if (!isExternalStatus && !sessionId) return false;
  ...
  const draw = (kind, message, detail = '') => {
    const success = kind === 'fulfilled'; const pending = kind === 'pending';
    card.innerHTML = `<div style="width:56px;height:56px;border-radius:50%;display:grid;place-items:center;background:${success ? '#DCFCE7' : pending ? '#FEF3C7' : '#FEE2E2'};color:${success ? '#15803D' : pending ? '#A16207' : '#B91C1C'};font-size:26px;margin-bottom:18px">${success ? '✓' : pending ? '…' : '!'}</div><h1 style="margin:0 0 10px;font-size:1.8rem;font-weight:800;color:#0F172A">${success ? 'Payment verified' : pending ? 'Waiting for payment confirmation' : 'Purchase status unavailable'}</h1><p style="margin:0;color:#475569;line-height:1.65">${message}</p>${detail ? `<p style="margin:14px 0 0;color:#64748B;font-size:.9rem;line-height:1.55">${detail}</p>` : ''}${success ? `<div style="margin-top:22px;display:flex;gap:10px;flex-wrap:wrap"><a href="#/library" class="btn btn-primary">Open My Library</a><a href="#/orders" class="btn btn-secondary">View Orders</a></div>` : ''}`;
  };
  ...
  const check = async () => {
    ...
    try {
      ...
      if (response.status === 404) { stopped = true; draw('error', 'This purchase session no longer exists. Please start the purchase again from the Bookora eBook page.'); return; }
...
function applyExternalDetailUI() {
  ...
  if (buy && /^https?:\/\//i.test(sourceUrl)) {
    ...
    buy.removeAttribute('href'); buy.removeAttribute('data-checkout'); buy.setAttribute('type', 'button');
    buy.onclick = async event => {
      event.preventDefault();
      const old = label?.textContent || 'Buy on Original Website';
      if (label) label.textContent = 'Preparing secure purchase…'; buy.disabled = true;
  ...
  if (purchase && sourceUrl && !purchase.querySelector('.bookora-external-disclosure')) {
    const note = document.createElement('div'); note.className = 'bookora-external-disclosure';
...
window.addEventListener('hashchange', run); window.addEventListener('load', run); run();
