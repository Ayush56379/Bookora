...
(()=>{
  ...
  const tags=()=>val('pub-tags','').split(',').map(s=>s.trim()).filter(Boolean);
  function cover(){const i=$('pub-cover'),f=i?.files?.[0];if(f&&f!==lastFile){if(coverUrl)URL.revokeObjectURL(coverUrl);coverUrl=URL.createObjectURL(f);lastFile=f;window.__BOOKORA_PREVIEW_FILE__=f}return coverUrl}
  function render(){
    ...
    const title=val('pub-title','Untitled eBook'),subtitle=val('pub-subtitle',''),author=val('pub-author'),category=val('pub-category'),description=val('pub-description'),pages=val('pub-pages'),price=val('pub-price'),sale=val('pub-saleprice',''),ts=tags(),cv=cover();
  ...
  const bodyObs=new MutationObserver(()=>start());bodyObs.observe(document.body,{childList:true,subtree:true});start();
})();
