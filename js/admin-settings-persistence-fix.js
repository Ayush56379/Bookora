...
function buildSettings() {
  ...
  return {
    ...
    payments: { cashfree_environment: value('set-cf-env', oldPayments.cashfree_environment || 'SANDBOX'), cashfree_app_id: value('set-cf-appid', oldPayments.cashfree_app_id || ''), api_version: value('set-cf-api-version', oldPayments.api_version || '2023-08-01') },
...
async function persistSettings(event) {
  ...
  if (!['SANDBOX', 'PRODUCTION'].includes(env)) { Toast.show('Invalid Cashfree environment.', 'error'); return; }
...
// ---- Persistent admin buyer/seller mode hotfix ----
// The selected mode is a UI preference, not something that should be reset by
// Firebase/profile/data synchronization. Keep the last explicit selection.
...
state.subscribe((event) => {
  ...
  if (event === 'MODE_CHANGED') {
    try {
      if (VALID_MODES.has(state.activeMode) && canUseMode(state.activeMode)) localStorage.setItem(MODE_KEY, state.activeMode);
    } catch (_) {}
  }
});
