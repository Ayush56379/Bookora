...
(() => {
  ...
  const collectDocuments = seller => {
    ...
    Object.entries(seller || {}).forEach(([key, value]) => {
      ...
      flatten(value, key).forEach(item => { if (item?.value != null && item.value !== '') entries.push(item); });
    });
    const seen = new Set();
  ...
  const renderDocuments = () => {
    ...
    if (!seller) return;
    content.querySelector('[data-bookora-seller-documents]')?.remove();
    const docs = collectDocuments(seller);
    const section = document.createElement('div');
    section.className = 'seller-detail-section';
    section.setAttribute('data-bookora-seller-documents', '1');
  ...
  const start = () => {
    if (!isRoute()) { stopListener(); return; }
    const db = getFirebase();
    if (db && !unsubscribe) {
      try { unsubscribe = db.collection(SELLER_COLLECTION).onSnapshot(snapshot => { sellers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); renderDocuments(); }, error => console.warn('[Bookora Seller Documents] Firestore listener:', error)); } catch (error) { console.warn('[Bookora Seller Documents] listener:', error); }
    }
