(()=>{
 ...
 const nodeInventory=(root,limit=900)=>{const nodes=[...root.querySelectorAll('body *')].filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0}).slice(0,limit);return nodes.map((e,i)=>({i,tag:e.tagName.toLowerCase(),id:e.id||'',classes:[...e.classList].slice(0,8),role:e.getAttribute('role')||'',text:(e.textContent||'').trim().replace(/\s+/g,' ').slice(0,140),selector:e.id?'#'+CSS.escape(e.id):e.tagName.toLowerCase()+(e.classList.length?'.'+[...e.classList].slice(0,2).map(CSS.escape).join('.'):''),rect:{w:Math.round(e.getBoundingClientRect().width),h:Math.round(e.getBoundingClientRect().height)}}))};
 const inventory=()=>nodeInventory(document,900);
 const sitePrompt=prompt=>{const p=String(prompt||'').toLowerCase();return /footer|header|navbar|navigation|menu|homepage|home page|website|site-wide|global|all pages|public page|book page|product page|link|icon|badge|responsive|mobile|desktop|design/.test(p)};
 let siteFrame=null,siteFramePromise=null;
 const publicSiteInventory=async(prompt)=>{if(!sitePrompt(prompt))return [];
   try{
     if(siteFrame?.contentDocument?.body?.children?.length)return nodeInventory(siteFrame.contentDocument,1100);
     if(!siteFramePromise){
       siteFramePromise=new Promise(resolve=>{
         const frame=document.createElement('iframe');siteFrame=frame;frame.setAttribute('aria-hidden','true');frame.style.cssText='position:fixed;left:-10000px;top:-10000px;width:1440px;height:1200px;opacity:0;pointer-events:none;border:0;z-index:-1';
         frame.src=location.origin+location.pathname+'#/';
         const finish=()=>{setTimeout(()=>resolve(frame),900)};
         frame.addEventListener('load',finish,{once:true});
         document.body.appendChild(frame);setTimeout(()=>resolve(frame),7000);
       });
     }
     const frame=await siteFramePromise;
     await new Promise(r=>setTimeout(r,1200));
     const doc=frame?.contentDocument;
     if(!doc?.body)return [];
     return nodeInventory(doc,1100);
   }catch(e){console.warn('[Bookora AI] public site inventory unavailable',e);return []}
 };
 const originalFetch=window.fetch.bind(window);
 window.fetch=async(input,init)=>{let url=typeof input==='string'?input:(input&&input.url)||'';if(!url.includes('/api/admin/ai-builder/prompts')||!init?.body)return originalFetch(input,init);try{const body=JSON.parse(init.body);if(body.action==='run'){const prompt=String(body.prompt||'');const current=inventory();const publicNodes=await publicSiteInventory(prompt);if(publicNodes.length){body.domInventory=[...publicNodes.map(x=>({...x,source:'public-site'})),...current.map(x=>({...x,source:'admin-control-panel'}))];body.pageContext=sitePrompt(prompt)?'#/' :(location.hash||location.pathname);body.targetContext=sitePrompt(prompt)?'public-site':'current-page'}else{body.domInventory=current;body.pageContext=location.hash||location.pathname}init={...init,body:JSON.stringify(body)}}}catch(e){console.warn('[Bookora AI] request context enrichment skipped',e)}const response=await originalFetch(input,init);try{const data=await response.clone().json();if(data?.job?.status==='success'){applyOps(data.job.domOps||[]);await refreshActive()}}catch{}return response};
