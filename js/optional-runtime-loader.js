...
// Publish page is intentionally isolated: its page module owns the complete
// upload lifecycle. Only lightweight navigation/file-selection helpers load here.
(() => {
  ...
  const common = ['./auth-logout-ui-permanent-fix.js?v=20260826-1','./backend-token-restore.js?v=20260823-3','./firebase-auth-token-bridge.js?v=20260821-9','./profile-menu-stability.js?v=20260822-1','./ai-single-trigger-global-cleanup.js?v=20260830-single-ai-v2'];
  const routeModules = {
  ...
  let generation=0;
  const loadForRoute=()=>{const myGeneration=++generation;if(!window.__BOOKORA_CORE_BOOTED__)return;const r=route();const items=selected();const run=async()=>{for(const src of items){if(myGeneration!==generation)return;await loadOne(src);if(r!=='/publish')await new Promise(resolve=>setTimeout(resolve,60));}};const start=()=>run().catch(error=>console.warn('[Bookora optional queue]',error));if(r==='/publish')start();else if('requestIdleCallback'in window)requestIdleCallback(start,{timeout:1800});else setTimeout(start,900);};
