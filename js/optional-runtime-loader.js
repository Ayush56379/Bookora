...
// IMPORTANT: never import dozens of optional modules concurrently. Their module
// evaluation can run synchronously and starve the main thread during navigation.
(() => {
  ...
  const loadOne = src => {
    ...
    const task = import(src).catch(error => {
      console.warn('[Bookora optional runtime skipped]', src, error);
    });
    return Promise.race([task, timeout]).catch(() => {});
  };

  // Keep concurrency intentionally low. A module may execute synchronous DOM/Firebase
  // setup while being imported, so dozens of parallel imports can freeze navigation.
  ...
  const start = () => {
    ...
    started = true;
    // Run optional enhancements progressively so route clicks remain responsive.
