...
// Core startup intentionally has no eager imports of page modules.
// A broken optional/page module must never blank the whole application.
...
const safeImport = async (path) => {
  try { return await import(path); }
    ...
    console.error('[Bookora route module failed]', path, error);
    throw error;
  }
};
...
class SafeApp {
  ...
  async loadPage(path, params) {
    if (path === '/' || path === '') {
      const m = await safeImport('./pages/HomePage.js');
    ...
    if (path === '/search') {
      const m = await safeImport('./pages/SearchPage.js');
      return { html: m.renderSearchPage(params.get('q') || '') };
    }
    if (path === '/categories') {
      const m = await safeImport('./pages/PublicDiscoveryPages.js');
      return { html: m.renderCategoriesDirectoryPage() };
    }
    if (path.startsWith('/category/')) {
      const m = await safeImport('./pages/CategoryPage.js');
      return { html: m.renderCategoryPage(path.replace('/category/','')) };
    }
    if (path.startsWith('/book/')) {
      const m = await safeImport('./pages/BookDetailPage.js');
      const slug = path.replace('/book/','');
      return { html: m.renderBookDetailPage(slug), init: () => m.initBookDetailEvents(slug) };
    }
    if (path.startsWith('/sample/')) {
      const m = await safeImport('./pages/FreeSamplePage.js');
      const slug = decodeURIComponent(path.replace('/sample/',''));
      const book = state.getBookBySlug(slug);
      if (!book) return { html: `<main class="container" style="padding:80px 20px;text-align:center"><h2>Sample is loading…</h2><p>Please return to the book and open the sample again.</p><a href="#/explore" class="btn btn-primary">Back to Explore</a></main>` };
      return { html: m.renderFreeSamplePage(book), init: () => m.initFreeSamplePage(book) };
    }
    if (['/best-sellers','/new-releases','/trending'].includes(path)) {
      const m = await safeImport('./pages/PublicDiscoveryPages.js');
    ...
    if (path === '/subscription/manage') {
      const m = await safeImport('./pages/SubscriptionManagePage.js');
      return { html: m.renderSubscriptionManagePage(), init: m.initSubscriptionManageEvents };
    }
    ...
    if (staticRoutes.includes(path)) {
      const m = await safeImport('./pages/StaticPages.js');
      return { html: m.renderStaticPage(path.slice(1)) };
    }
    if (['/login','/signup','/register','/forgot-password','/reset-password'].includes(path)) {
      const m = await safeImport('./pages/AuthPages.js');
      const mode = path === '/login' ? 'login' : ['/signup','/register'].includes(path) ? 'signup' : path === '/forgot-password' ? 'forgot' : 'reset';
      return { html: m.renderAuthPage(mode), init: () => m.initAuthEvents(mode) };
    }
    if (path === '/profile') {
      const m = await safeImport('./pages/ProfilePage.js');
      return { html: m.renderProfilePage() };
    }
    if (['/settings','/settings/account','/settings/notifications','/settings/privacy'].includes(path)) {
      const m = await safeImport('./pages/UserSettingsPage.js');
      return { html: m.renderUserSettingsPage(), init: m.initUserSettingsEvents };
    }
    if (path === '/settings/security') {
      const m = await safeImport('./pages/AccountSecurityPage.js');
      return { html: m.renderAccountSecurityPage(), init: m.initAccountSecurityEvents };
    }
    if (path === '/dashboard') {
      const m = await safeImport('./pages/DashboardPage.js');
      return { html: m.renderDashboardPage(), init: m.initDashboardEvents };
    }
    if (path === '/library') {
      const m = await safeImport('./pages/LibraryPage.js');
      return { html: m.renderLibraryPage(), init: m.initLibraryEvents };
    }
    if (path === '/orders') {
      const m = await safeImport('./pages/OrdersPage.js');
      return { html: m.renderOrdersPage() };
    }
    if (path === '/wishlist') {
      const m = await safeImport('./pages/WishlistPage.js');
      return { html: m.renderWishlistPage() };
    }
    if (path.startsWith('/checkout/')) {
      const m = await safeImport('./pages/CheckoutPage.js');
      const slug = path.replace('/checkout/','');
      return { html: m.renderCheckoutPage(slug), init: () => m.initCheckoutEvents(slug) };
    }
    if (path === '/payment/success') {
      const m = await safeImport('./pages/PaymentSuccessPage.js');
      return { html: m.renderPaymentSuccessPage() };
    }
    if (path === '/payment/failed') {
      const m = await safeImport('./pages/PaymentFailedPage.js');
      return { html: m.renderPaymentFailedPage() };
    }
    if (['/seller','/seller/dashboard','/creator','/creator/dashboard'].includes(path)) {
      const m = await safeImport('./pages/CreatorDashboardPage.js');
      return { html: m.renderCreatorDashboardPage(), init: m.initCreatorDashboardEvents };
    }
    if (path === '/publish') {
    ...
    if (path === '/seller/settings') {
      const m = await safeImport('./pages/SellerSettingsPage.js');
      return { html: m.renderSellerSettingsPage(), init: m.initSellerSettingsEvents };
    }
    if (path === '/admin' || path === '/admin/overview') {
      const m = await safeImport('./pages/AdminDashboardPage.js');
      return { html: m.renderAdminDashboardPage(), init: m.initAdminDashboardEvents };
    }
    ...
    const m = await safeImport('./pages/NotFoundPage.js');
    return { html: m.renderNotFoundPage() };
  }
  ...
  async route(force = false, navigation = false) {
    ...
    try {
      ...
      try {
        const page = await this.loadPage(path, params);
        const main = document.getElementById('main-content');
        if (!main) throw new Error('main-content was not created');
        main.innerHTML = page.html || '';
        if (typeof page.init === 'function') {
          try { await page.init(); } catch (error) { console.warn('[Bookora page events skipped]', error); }
        }
        try {
          const h = await safeImport('./components/Header.js');
          if (typeof h.initHeaderEvents === 'function') h.initHeaderEvents();
        } catch (error) { console.warn('[Bookora header events skipped]', error); }
      } catch (error) {
