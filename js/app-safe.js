...
class SafeApp {
  ...
  async loadPage(path,params){
    ...
    if(path==='/publish'){const m=await safeImport('./pages/PublishInternalPage.js?v=20260831-publish-v5');return{html:m.renderPublishInternalPage(),init:m.initPublishInternalEvents};}
    ...
    if(path==='/admin/orders'){const m=await safeImport('./pages/AdminOrdersPage.js');return{html:m.renderAdminOrdersPage()};}
  ...
  async route(force=false,navigation=false){
    ...
    try{const path=this.currentPath();const [,queryString]=hash.split('?');const params=new URLSearchParams(queryString||'');if(navigation)window.scrollTo({top:0,left:0,behavior:'instant'});const PUBLIC=['/','/explore','/categories','/best-sellers','/new-releases','/trending','/authors','/pricing','/subscription','/about','/how-it-works','/faq','/contact','/help','/terms','/privacy','/refund-policy','/seller-guidelines','/login','/signup','/register','/forgot-password','/reset-password','/payment/success','/payment/failed','/review-support'];const PUBLIC_PREFIX=['/category/','/book/','/author/','/search','/sample/'];const isPublic=PUBLIC.includes(path)||PUBLIC_PREFIX.some(p=>path.startsWith(p));if(!isPublic&&!state.isAuthenticated){window.location.hash=`#/login?returnTo=${encodeURIComponent(path+(queryString?`?${queryString}`:''))}`;return;}if(path.startsWith('/admin')&&!state.isAdmin){window.location.hash='#/login';return;}if((path.startsWith('/seller')||path.startsWith('/creator')||path==='/publish'||path.startsWith('/publish/external'))&&path!=='/seller/apply'&&!state.isSeller&&!state.isAdmin){window.location.hash='#/seller/apply';return;}
      const {header,footer}=await this.shell();this.root.innerHTML=`${header}<main id="main-content" style="flex:1;min-height:60vh"><div aria-label="Loading Bookora" style="min-height:60vh;display:grid;place-items:center;text-align:center;font-family:Inter,system-ui,sans-serif;font-size:64px;font-weight:600;letter-spacing:-0.055em;color:#12a879;user-select:none">Bookora</div></main>${footer}`;
      try{const page=await this.loadPage(path,params);const main=document.getElementById('main-content');if(!main)throw new Error('main-content was not created');main.innerHTML=page.html||'';if(typeof page.init==='function'){try{await page.init();}catch(error){console.warn('[Bookora page events skipped]',error);}}try{const h=await safeImport('./components/Header.js');if(typeof h.initHeaderEvents==='function')h.initHeaderEvents();}catch(error){console.warn('[Bookora header events skipped]',error);}}catch(error){console.error('[Bookora] route failed:',error);const main=document.getElementById('main-content');if(main)main.innerHTML=`<div role="alert" style="min-height:60vh;display:grid;place-items:center;text-align:center;font-family:Inter,system-ui,sans-serif;font-size:20px;font-weight:700;color:#b91c1c;padding:30px">Publish page could not be loaded. Please refresh and try again.</div>`;}
      this.lastHash=hash;
    }catch(error){console.error('[Bookora] core route failed:',error);this.root.innerHTML=`<main id="main-content" style="min-height:60vh;display:grid;place-items:center;text-align:center;font-family:Inter,system-ui,sans-serif;font-size:20px;font-weight:700;color:#b91c1c;padding:30px">Bookora could not load this page. Please refresh and try again.</main>`;}
