// Bookora quick seller onboarding route override.
// Replaces the long legacy seller form without touching other routes.
(() => {
  ...
  const install = () => {
    ...
    app.loadPage = async (path, params) => {
      if (path === '/seller/apply') {
        const m = await import('./pages/SellerApplyQuickPage.js?v=20260827-quick1');
