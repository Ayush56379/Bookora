...
async function renderPdf(stack,payload,requestId){
  ...
  // The page grid is two columns on desktop and one column on small screens.
  // Render each PDF page at the width of its actual grid cell. The previous
  // implementation rendered every page at the full 780px stack width, then
  // placed those canvases into ~382px grid cells, which caused the right side
  // of every page to be visibly cropped.
  ...
  for(let i=0;i<count;i++){
    ...
    await page.render({canvasContext:context,viewport:vp,transform:ratio!==1?[ratio,0,0,ratio,0,0]:undefined}).promise;
