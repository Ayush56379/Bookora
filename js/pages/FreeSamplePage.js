...
async function renderPdf(stack,payload,requestId){
  ...
  stack.innerHTML='';
  const width=Math.min(780,stack.clientWidth||780),ratio=Math.min(devicePixelRatio||1,2),frag=document.createDocumentFragment();
  for(let i=0;i<count;i++){
    if(requestId!==stack.dataset.requestId){pdf.destroy?.();return;}
    const page=await pdf.getPage(i+1),base=page.getViewport({scale:1}),vp=page.getViewport({scale:width/base.width}),canvas=document.createElement('canvas');
  ...
  if(requestId===stack.dataset.requestId)stack.appendChild(frag);
  pdf.destroy?.();
}
...
async function requestPreview(book,signal){
  ...
  let response;
  try{
    // Free samples are public. Do not call /api/auth/me and do not send a
    // stale browser session/token just to render a public preview.
    response=await fetch(url,{cache:'no-store',credentials:'omit',headers:{Accept:'application/json'},signal});
  }catch(error){
    console.error('[Bookora Free Sample] API network failure',meta,{error:error?.message});
    throw Error(`PREVIEW_NETWORK_ERROR: ${error?.message||'request failed'}`);
  }
  const payload=await readJsonResponse(response);
  if(!response.ok||payload?.success!==true){
    console.error('[Bookora Free Sample] API failed',{...meta,status:response.status,response:payload});
    throw Error(`${payload?.error||`HTTP_${response.status}`}: ${payload?.message||'Preview pages could not be generated.'}`);
  }
  if(payload.bookId&&String(payload.bookId)!==key&&String(payload.bookId)!==String(book?.id||'')){
    console.error('[Bookora Free Sample] API book mismatch',{...meta,responseBookId:payload.bookId});
    throw Error('PREVIEW_BOOK_MISMATCH');
  }
  console.info('[Bookora Free Sample] API success',{...meta,status:response.status,pageCount:payload.pageCount,limited:payload.limited});
  return payload;
}
