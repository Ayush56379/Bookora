...
function pendingMarkup(id,data){return shell('◷','Payment Pending','Cashfree has not confirmed a completed payment yet. Your eBook remains locked until Bookora receives a verified successful payment.','',details(id,data)+'<div style="margin-top:1rem;color:var(--text-muted);font-size:.85rem">We will keep checking automatically.</div>');}
function pendingFinalMarkup(id,data){return shell('◷','Payment Still Pending','We could not confirm a completed payment yet. Please check My Orders before trying again.', '<button id="payment-refresh-status" class="btn btn-primary btn-lg">Check Again</button><a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-secondary btn-lg">Continue Shopping</a>',details(id,data));}
function successMarkup(id,data){return shell('✓','Payment Successful','Your payment has been verified by Bookora. Your eBook access is now unlocked and the purchase has been added to your Library.','<a href="#/library" class="btn btn-primary btn-lg">Open My Library</a><a href="#/orders" class="btn btn-secondary btn-lg">View Order</a><a href="#/explore" class="btn btn-secondary btn-lg">Continue Shopping</a>',details(id,data));}
function cancelledMarkup(id,data){return shell('×','Payment Cancelled','The Cashfree payment was cancelled. No eBook was unlocked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
function failedMarkup(id,data){return shell('!','Payment Failed','Cashfree did not confirm this payment. Your eBook remains locked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
function expiredMarkup(id,data){return shell('⌛','Payment Expired','This Cashfree payment session expired before Bookora received a successful payment. Your eBook remains locked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
...
async function verifyOrder(orderId){
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    const response=await fetch(`${api}/api/cashfree/verify-order?order_id=${encodeURIComponent(orderId)}`,{headers:{Accept:'application/json',Authorization:`Bearer ${state.token}`},cache:'no-store'});
    const data=await response.json().catch(()=>({}));
...
function attachRefresh(flow){document.getElementById('payment-refresh-status')?.addEventListener('click',()=>{flow.polls=0;flow.running=false;flow.done=false;runFlow(flow.orderId,true);},{once:true});}
...
async function runFlow(orderId,immediate=false){
  ...
  try{
    // Loading is rendered once by renderPaymentSuccessPage. Never render it during polling.
    ...
    render(pendingMarkup(orderId,data));flow.polls++;window.setTimeout(()=>runFlow(orderId),2000);
  }catch(error){console.error('Bookora payment verification:',error);flow.running=false;render(errorMarkup(error?.message));attachRefresh(flow);}
}
...
export function initPaymentSuccessEvents(){
  ...
  const flow=existing||{orderId,status:'PENDING',polls:0,running:false,done:false};flows.set(orderId,flow);runFlow(orderId);
}
