...
function extractStatus(data) {
  const values = [
    ...
    data?.cashfree?.payment?.payment_status,
...
function details(orderId, data) {
  ...
  const amount = cf?.order_amount ?? payment?.payment_amount ?? order?.order_amount ?? order?.amount;
  const currency = cf?.order_currency || payment?.payment_currency || 'INR';
  const gatewayOrder = cf?.cf_order_id || cf?.order_id || order?.cf_order_id || order?.cashfree_order_id || '—';
  const paymentId = payment?.cf_payment_id || order?.payment_id || '—';
  const method = payment?.payment_group || payment?.payment_method || '—';
  const paymentStatus = payment?.payment_status || cf?.payment_status || data?.payment_status || '—';
  const orderStatus = cf?.order_status || order?.order_status || '—';
  const paidAt = payment?.payment_completion_time || payment?.payment_time || order?.paid_at || '—';
...
function pendingMarkup(id,data){return shell('◷','Payment Pending','Cashfree has not confirmed a completed payment yet. Your eBook remains locked until Bookora receives a verified successful payment.','',details(id,data)+'<div style="margin-top:1rem;color:var(--text-muted);font-size:.85rem">Checking the verified Cashfree status automatically.</div>');}
function pendingFinalMarkup(id,data){return shell('◷','Payment Verification Delayed','Cashfree has not confirmed a completed payment yet. Please check again before starting another payment.','<button id="payment-refresh-status" class="btn btn-primary btn-lg">Check Again</button><a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-secondary btn-lg">Continue Shopping</a>',details(id,data));}
function successMarkup(id,data){return shell('✓','Payment Successful','Your payment has been verified by Cashfree and Bookora. Your eBook access is unlocked and the purchase has been added to your Library.','<a href="#/library" class="btn btn-primary btn-lg">Open My Library</a><a href="#/orders" class="btn btn-secondary btn-lg">View Order</a><a href="#/explore" class="btn btn-secondary btn-lg">Continue Shopping</a>',details(id,data));}
function cancelledMarkup(id,data){return shell('×','Payment Cancelled','The Cashfree payment was cancelled or dropped. No eBook was unlocked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
function failedMarkup(id,data){return shell('!','Payment Failed','Cashfree did not confirm a successful payment. Your eBook remains locked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
function expiredMarkup(id,data){return shell('⌛','Payment Expired','This Cashfree payment session expired before a successful payment was confirmed. Your eBook remains locked.','<a href="#/orders" class="btn btn-secondary btn-lg">My Orders</a><a href="#/explore" class="btn btn-primary btn-lg">Try Again</a>',details(id,data));}
...
async function fetchJsonWithTimeout(url, options={}, timeoutMs=12000){
  ...
  try {
    ...
    if (error?.name === 'AbortError') throw new Error('Payment verification timed out. Please check again.');
    throw error;
