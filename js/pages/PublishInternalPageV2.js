...
async function request(path, options={}, retry=true) {
  ...
  try {
    let r = await fetch(`${API}${path}`, { ...options, headers, signal: controller.signal });
...
function fileUI() {
  ...
  const pc = $('pdf-selected-card'), cc = $('cover-selected-card');
  pc?.classList.toggle('ready', !!selectedPDF); cc?.classList.toggle('ready', !!selectedCover);
}
...
function renderPreview() {
  ...
  const description=val('pub-description','—'), pages=val('pub-pages','—'), price=val('pub-price','0'), sale=val('pub-saleprice');
...
async function uploadFile(file, kind) {
  ...
  try {
    const r=await fetch(session.upload_url,{method:'PUT',body:file,signal:c.signal});
    if(!r.ok) throw new Error(kind==='pdf'?'PDF upload failed. Please retry.':'Cover upload failed. Please retry.');
...
export async function initPublishInternalEvents() {
  ...
  if(!configLoaded){configLoaded=true; try { const r=await fetch(`${API}/api/settings/public`,{headers:{Accept:'application/json'}}); if(r.ok){const d=await r.json();const limit=Number(d?.books_config?.max_pdf_size_mb);if(Number.isFinite(limit)&&limit>0&&limit<MAX_PDF_MB){const t=document.querySelector('.upload-card p');if(t)t.textContent=`PDF only · maximum ${Math.floor(limit)} MB`;}}}catch(_){} }
}
