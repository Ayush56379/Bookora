// Bookora HomePage — buyer-first marketplace with live Firebase catalog updates
...
function normalizeFastBooks() {
  const fast = Array.isArray(window.__BOOKORA_FAST_BOOKS__) ? window.__BOOKORA_FAST_BOOKS__ : [];
  return fast.map(book => state.normalizeBook(book)).filter(Boolean).filter(book => book.status === 'approved');
}

function getFeaturedBooks() {
  const stateBooks = state.getApprovedBooks();
  const fastBooks = normalizeFastBooks();
  const books = stateBooks.length ? stateBooks : fastBooks;
...
function renderCatalogContent() {
  ...
  catalog.innerHTML = featured.length
    ...
    : `<div class="home-empty-state"><div class="home-empty-icon">📚</div><h3>Loading the latest eBooks…</h3><p>Bookora is connecting to the catalog.</p></div>`;
}
...
export function initHomePageEvents() {
  ...
  // Firebase fast listener and the normal state sync both update only the catalog region.
  const refreshCatalog = () => renderCatalogContent();
  window.addEventListener('bookora:fast-catalog', refreshCatalog);
  window.addEventListener('bookora:catalog-updated', refreshCatalog);

  const cleanup = () => {
    window.removeEventListener('bookora:fast-catalog', refreshCatalog);
    window.removeEventListener('bookora:catalog-updated', refreshCatalog);
  };
  window.addEventListener('hashchange', cleanup, { once: true });

  const reveal = () => document.querySelectorAll('.home-reveal,.home-book-item').forEach(el => {
    if (el.dataset.homeVisible === '1') return;
    if (el.getBoundingClientRect().top < window.innerHeight * .94) {
      el.dataset.homeVisible = '1';
...
if (!document.getElementById('bookora-clean-home-styles')) {
  ...
  style.textContent = `
    ...
    @media(max-width:1050px){.home-book-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.home-hero-inner{gap:1rem}.home-art-device{right:0}}@media(max-width:800px){.home-hero-video{min-height:auto;border-radius:0 0 24px 24px}.home-hero-inner{grid-template-columns:1fr;text-align:center;padding:4rem 1rem}.home-hero-copy{margin:auto}.home-hero-copy>p{margin-left:auto;margin-right:auto}.home-hero-actions{justify-content:center}.home-search-clean{margin:auto}.home-hero-art{height:310px}.home-book-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.home-trust-grid{grid-template-columns:1fr}}@media(max-width:520px){.home-hero-inner{padding:3.2rem .85rem}.home-hero-copy h1{font-size:clamp(2.65rem,12vw,3.6rem)}.home-hero-copy>p{font-size:.92rem}.home-primary-action,.home-secondary-action{padding:.78rem .85rem;font-size:.78rem}.home-search-clean{height:52px;padding-left:.85rem}.home-search-clean button{padding:.62rem .8rem}.home-search-clean input{font-size:.8rem}.home-hero-art{height:255px;transform:scale(.86)}.home-art-device{right:-4px;top:52px}.home-floating-page{display:none}.home-catalog-clean{padding:3.5rem 0}.home-section-head{align-items:flex-start;flex-direction:column}.home-book-grid{gap:.75rem}}@media(prefers-reduced-motion:reduce){.home-reveal,.home-book-item,.home-art-card,.home-art-front,.home-art-device,.home-floating-page,.home-art-orbit,.home-video-bg{animation:none!important;transition:none!important;opacity:1;transform:none}}
  `;
