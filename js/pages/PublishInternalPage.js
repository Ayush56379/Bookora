...
export function initPublishInternalEvents(){
  ...
  form.addEventListener('click',e=>{const n=e.target.closest('.next-step-btn'),p=e.target.closest('.prev-step-btn');if(n){e.preventDefault();const step=Number(n.dataset.next);if(step===2&&!validate1())return;if(step===3&&!validate2())return;if(step===4&&!validate3())return;if(step===4||step===5)preview();showStep(step);}else if(p){e.preventDefault();showStep(Number(p.dataset.prev));}});
