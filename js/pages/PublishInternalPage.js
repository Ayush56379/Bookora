...
async function loadUploadConfig() {
  ...
  if (Number.isFinite(local) && local > 0) {
    uploadConfig.maxPdfMb = Math.min(MAX_ADMIN_PDF_MB, Math.max(1, Math.floor(local)));
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (Number.isFinite(remote) && remote > 0) {
        uploadConfig.maxPdfMb = Math.min(MAX_ADMIN_PDF_MB, Math.max(1, Math.floor(remote)));
      }
    }
  } catch (error) {
    console.warn('Upload limit refresh skipped:', error);
  }
...
function validateStep2() {
  ...
  const maxBytes = uploadConfig.maxPdfMb * 1024 * 1024;
  if (selectedPDF.size > maxBytes) {
    Toast.show(`PDF must be ${uploadConfig.maxPdfMb} MB or smaller.`, 'warning');
    return false;
  }
  ...
  if (selectedCover.size > MAX_COVER_MB * 1024 * 1024) {
    Toast.show(`Cover must be ${MAX_COVER_MB} MB or smaller.`, 'warning');
    return false;
  }
...
function showStep(step) {
  ...
  document.querySelectorAll('.wizard-step-node').forEach(node => {
    ...
    if (number === targetStep) {
      circle.style.background = 'var(--accent)'; circle.style.color = '#fff'; circle.style.borderColor = 'var(--accent)'; title.style.color = 'var(--accent)';
    } else if (number < targetStep) {
      circle.style.background = '#ECFDF5'; circle.style.color = '#059669'; circle.style.borderColor = '#059669'; title.style.color = '#059669';
    } else {
      circle.style.background = '#fff'; circle.style.color = 'var(--text-muted)'; circle.style.borderColor = 'var(--border-medium)'; title.style.color = 'var(--text-muted)';
    }
  });
  if (targetStep === 3) updateRoyalty();
  if (targetStep === 4) requestAnimationFrame(updatePreview);
...
function setSubmitProgress(text, percent = null) {
  ...
  if (label && percent !== null) label.textContent = `${text} ${Math.round(percent)}%`;
...
function uploadKey(file, kind) {
  return `bookora_upload_${kind}_${file.name}_${file.size}_${file.lastModified}`;
}
...
async function startUpload(file, kind) {
  const data = await requestJson('/api/books/upload-session/start', {
    method: 'POST',
    headers: authHeaders(),
...
async function uploadFileResumable(file, kind, onProgress) {
  const storageKey = uploadKey(file, kind);
  let stateData = null;
  try {
    const cached = sessionStorage.getItem(storageKey);
    if (cached) stateData = JSON.parse(cached);
  } catch (_) {}

  let session = stateData?.token ? { token: stateData.token, chunkSize: Number(stateData.chunkSize) || 2 * 1024 * 1024, offset: 0 } : null;
  let restartCount = 0;
  ...
  while (restartCount <= 2) {
    try {
      ...
      while (session.offset < file.size) {
        ...
        const data = base64FromArrayBuffer(chunk);
        let completed = false;
        let attempts = 0;

        while (!completed && attempts < UPLOAD_RETRIES) {
          attempts += 1;
          try {
            const response = await requestJson('/api/books/upload-session/chunk', {
              method: 'POST',
              headers: authHeaders(),
              body: JSON.stringify({ upload_token: session.token, offset: start, data })
            });
            ...
            if (error.status === 409 && Number(error.data?.next_offset) >= 0) {
              session.offset = Math.min(file.size, Number(error.data.next_offset));
              completed = true;
              onProgress(session.offset / file.size);
              break;
            }
            if (error.status === 410) throw error;
            if (attempts >= UPLOAD_RETRIES) throw error;
            await new Promise(resolve => setTimeout(resolve, Math.min(5000, 700 * attempts)));
          }
        }
      }

      const finalStatus = await requestJson('/api/books/upload-session/status', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ upload_token: session.token })
      });
      ...
      if (error.status === 410 && restartCount < 2) {
        session = null;
        restartCount += 1;
    ...
    <div class="publish-page" style="background:var(--bg-secondary);min-height:85vh;padding:2rem 0 5rem;">
      <div class="container" style="max-width:900px;">
        ...
        <div style="display:flex;gap:.35rem;justify-content:space-between;margin-bottom:2rem;">
          ${['Info','Files','Pricing','Preview','Submit'].map((step,index)=>`<div class="wizard-step-node" data-step="${index+1}" style="text-align:center;flex:1;"><div class="step-num" style="width:36px;height:36px;border-radius:50%;background:${index===0?'var(--accent)':'#fff'};color:${index===0?'#fff':'var(--text-muted)'};border:2px solid ${index===0?'var(--accent)':'var(--border-medium)'};display:flex;align-items:center;justify-content:center;font-weight:700;margin:auto;">${index+1}</div><span class="step-title" style="font-size:.72rem;font-weight:600;">${index+1}. ${step}</span></div>`).join('')}
        </div>
        <div style="background:#fff;border:1px solid var(--border-subtle);border-radius:var(--radius-xl);padding:clamp(1rem,4vw,2.5rem);box-shadow:var(--shadow-sm);">
          <form id="publish-wizard-form">
            ...
            <section id="step-2" class="wizard-section" style="display:none;">
              ...
              <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1rem;align-items:end;margin-bottom:1.5rem;"><div><label>Page Count *</label><input id="pub-pages" type="number" min="1" required placeholder="Automatically detected" style="width:100%;padding:.75rem;"></div><button type="button" id="detect-pages-btn" class="btn btn-secondary" style="white-space:nowrap;">Detect Pages</button></div>
              <div style="margin-bottom:1.5rem;"><label>Format</label><input id="pub-format" value="PDF" readonly style="width:100%;padding:.75rem;background:#f8fafc;"></div>
              <div style="display:flex;justify-content:space-between;gap:1rem;"><button type="button" class="btn btn-secondary prev-step-btn" data-prev="1">← Back</button><button type="button" class="btn btn-primary next-step-btn" data-next="3">Next: Pricing →</button></div>
            </section>
...
export function initPublishInternalEvents() {
  ...
  pdfInput?.addEventListener('change', async event => {
    ...
    if (!file.name.toLowerCase().endsWith('.pdf') && file.type !== 'application/pdf') {
      Toast.show('Please select a PDF file.', 'warning'); event.target.value = ''; return;
    }
    if (file.size > uploadConfig.maxPdfMb * 1024 * 1024) {
      Toast.show(`PDF must be ${uploadConfig.maxPdfMb} MB or smaller.`, 'warning'); event.target.value = ''; return;
    }
    const validSignature = await validatePdfSignature(file);
    if (!validSignature) {
      Toast.show('The selected file is not a valid PDF.', 'warning'); event.target.value = ''; return;
    }
    selectedPDF = file;
    updateFilesUI();
  ...
  coverInput?.addEventListener('change', event => {
    ...
    if (!['image/jpeg','image/png','image/webp'].includes(file.type)) {
      Toast.show('Please select a JPG, PNG or WEBP cover.', 'warning'); event.target.value = ''; return;
    }
    if (file.size > MAX_COVER_MB * 1024 * 1024) {
      Toast.show(`Cover must be ${MAX_COVER_MB} MB or smaller.`, 'warning'); event.target.value = ''; return;
    }
    selectedCover = file;
    updateFilesUI();
  ...
  detectPagesButton?.addEventListener('click', () => detectAndSetPages());
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      if (!validateStep2()) throw new Error('The upload limit changed. Please re-check your files.');
      ...
      let uploadedBytes = 0;
      const progress = (bytes, label) => {
        uploadedBytes += bytes;
        const percent = Math.min(100, uploadedBytes / totalBytes * 100);
        setSubmitProgress(label, percent);
      };

      setSubmitProgress('Uploading PDF...', 0);
      const pdfFile = await uploadFileResumable(selectedPDF, 'pdf', ratio => {
        const percent = (ratio * selectedPDF.size / totalBytes) * 100;
        setSubmitProgress('Uploading PDF...', percent);
      });
      uploadedBytes = selectedPDF.size;
      progress(0, 'PDF uploaded. Starting cover...');

      const coverFile = await uploadFileResumable(selectedCover, 'cover', ratio => {
        const percent = ((selectedPDF.size + ratio * selectedCover.size) / totalBytes) * 100;
        setSubmitProgress('Uploading cover...', percent);
      });
      uploadedBytes = totalBytes;
      setSubmitProgress('Files uploaded. Creating listing...', 100);

      const bookResponse = await requestJson('/api/books/create', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({
          action: 'createBook', title, subtitle, author, category, description, tags, pages,
          format: 'PDF', price, sale_price: salePrice,
          ...
          cover_file_id: coverFile?.id || coverFile?.file_id || coverFile?.fileId || '',
          pdf_file_id: pdfFile?.id || pdfFile?.file_id || pdfFile?.fileId || '',
          status: 'pending'
        })
      });
      ...
      selectedCover = null;
      try {
        sessionStorage.removeItem(uploadKey(selectedPDF, 'pdf'));
        sessionStorage.removeItem(uploadKey(selectedCover, 'cover'));
      } catch (_) {}
      setTimeout(() => { window.location.hash = '#/creator/dashboard'; }, 800);
    } catch (error) {
      console.error('Publish eBook error:', error);
      Toast.show(error?.message || 'Unable to publish eBook. Your upload can be retried safely.', 'error');
      if (submitButton) submitButton.disabled = false;
      setSubmitProgress('Retry upload', 0);
    }
  });
