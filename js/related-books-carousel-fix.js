...
const STYLE_ID = 'bookora-related-carousel-fix-v2';
let applyTimer = 0;
let applying = false;
let observer = null;
let pollTimer = 0;
...
function scoreRelated(book, candidate) {
  ...
  const candidateWords = `${candidateCategory} ${normalize(candidate?.title)} ${candidateTags.join(' ')}`.split(/\s+/).filter(word => word.length >= 4);
  ...
  return (exactCategory ? 1000 : 0) + (relatedCategory ? 450 : 0) + (tagMatch ? 250 : 0) + Math.min(wordMatch, 5) * 35 + (sameSource ? 8 : 0);
}
...
function addStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    #app .bd-page .bd-related-carousel{position:relative;min-width:0;contain:layout style;}
    ...
    #app .bd-page .bd-related > *{flex:0 0 clamp(220px,23vw,260px)!important;width:clamp(220px,23vw,260px)!important;min-width:clamp(220px,23vw,260px)!important;max-width:clamp(220px,23vw,260px)!important;scroll-snap-align:start!important;}
...
function decorateRatings(items) {
  return items.map(item => {
    const rating = ratingData(item);
    return rating.rating > 0 ? { ...item, rating: rating.rating, review_count: rating.count } : item;
  });
}
...
function applyRelatedCarousel() {
  ...
  const book = state.getApprovedBooks().find(item => String(item.id) === String(currentId)) || state.getBookBySlug(window.location.hash.split('/').pop()?.split('?')[0] || '');
  if (!book) return;
...
function watchUntilRelatedExists() {
  stopObserver();
  const app = document.getElementById('app') || document.body;
  const tryApply = () => {
    applyRelatedCarousel();
    if (document.querySelector('#app .bd-page .bd-related')) {
      stopObserver();
      return;
    }
    pollTimer = window.setTimeout(tryApply, 120);
  };
...
function scheduleApply() {
  ...
  applyTimer = window.setTimeout(() => { applyTimer = 0; requestAnimationFrame(applyRelatedCarousel); }, 0);
}
...
function preserveDetailScroll() {
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    if (document.querySelector('#app .bd-page')) window.scrollTo({ top: y, left: window.scrollX, behavior: 'auto' });
  }));
}

addStyles();
watchUntilRelatedExists();
