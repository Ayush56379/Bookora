...
const STYLE_ID = 'bookora-related-carousel-stable-v5';
let observer = null;
let pollTimer = 0;
let running = false;
...
function ratingData(book) {
  ...
  const reviews = Array.isArray(state.reviews) ? state.reviews.filter(review => String(review.book_id || review.bookId) === String(book?.id)) : [];
...
function addStyles() {
  ...
  style.textContent = `
    ...
    #app .bd-page .bd-related{display:flex!important;grid-template-columns:none!important;gap:16px!important;overflow-x:auto!important;overflow-y:hidden!important;scroll-behavior:smooth!important;scroll-snap-type:none!important;overscroll-behavior-x:contain!important;overflow-anchor:none!important;padding:2px 48px 10px 2px!important;min-width:0!important;touch-action:pan-x!important;-webkit-overflow-scrolling:touch!important;}
    #app .bd-page .bd-related > *{flex:0 0 clamp(220px,23vw,260px)!important;width:clamp(220px,23vw,260px)!important;min-width:clamp(220px,23vw,260px)!important;max-width:clamp(220px,23vw,260px)!important;}
    #app .bd-page .bd-related-next{position:absolute!important;z-index:20!important;top:50%!important;right:7px!important;transform:translateY(-50%)!important;width:44px!important;height:44px!important;border:1px solid #dbe3ee!important;border-radius:50%!important;background:rgba(255,255,255,.97)!important;color:#0f172a!important;display:grid!important;place-items:center!important;cursor:pointer!important;box-shadow:0 8px 22px rgba(15,23,42,.14)!important;backdrop-filter:blur(8px)!important;}
    ...
    @media(max-width:600px){#app .bd-page .bd-related{gap:12px!important;padding:2px 46px 8px 2px!important;}#app .bd-page .bd-related > *{flex-basis:clamp(190px,72vw,230px)!important;width:clamp(190px,72vw,230px)!important;min-width:clamp(190px,72vw,230px)!important;max-width:clamp(190px,72vw,230px)!important;}#app .bd-page .bd-related-next{width:40px!important;height:40px!important;right:4px!important;}}
...
function getSelectedBook(page) {
  const currentId = page.dataset.bookId;
  if (currentId) {
    const byId = state.getApprovedBooks().find(item => String(item.id) === String(currentId));
    if (byId) return byId;
  }
  const slug = window.location.hash.split('/').pop()?.split('?')[0] || '';
  return state.getBookBySlug(slug);
}

function renderStableRelated() {
  if (running) return true;
  const page = document.querySelector('#app .bd-page');
  const related = page?.querySelector('.bd-related');
  if (!page || !related) return false;
  if (related.dataset.stableReady === '1') return true;

  const book = getSelectedBook(page);
  ...
  try {
    ...
    related.dataset.stableReady = '1';
    related.dataset.relatedSignature = candidates.map(item => String(item?.id || item?.slug || item?.title || '')).join('|');
    ...
    if (!carousel.querySelector('.bd-related-next')) {
      ...
      next.addEventListener('click', () => related.scrollBy({ left: Math.max(260, related.clientWidth * 0.8), behavior: 'smooth' }));
      carousel.appendChild(next);
    }
    requestAnimationFrame(() => { related.style.visibility = 'visible'; });
    return true;
  } finally {
    running = false;
  }
...
function startWatcher() {
  ...
  const tryApply = () => {
    ...
    if (renderStableRelated()) { stopWatcher(); return; }
    pollTimer = window.setTimeout(tryApply, 150);
  };
  observer = new MutationObserver(() => {
    if (!running && !document.querySelector('#app .bd-page .bd-related[data-stable-ready="1"]')) tryApply();
  });
