...
class App {
  ...
  init() {
    ...
    state.subscribe((event) => {
      this.updateHeader();
      if (event === 'USER_LOGGED_IN' || event === 'USER_LOGGED_OUT' || event === 'MODE_CHANGED') this.route();
    });
    document.addEventListener('click', (e) => {
      ...
      if (wishBtn) {
        ...
        }).catch(err => { console.error('Wishlist error:', err); Toast.show('Unable to update wishlist.', 'error'); });
        return;
      }
  ...
  route() {
    window.scrollTo(0, 0);
    this.root = this.root && this.root.innerHTML ? this.root : (document.getElementById('app') || document.body);
