...
export function getCoverUrl(book) {
  ...
  if (/drive\.google\.com/i.test(raw) && idMatch?.[1]) {
    return `https://drive.google.com/thumbnail?id=${encodeURIComponent(idMatch[1])}&sz=w1600`;
  }
    ...
    <article class="book-card book-card-premium animate-fade-in" data-book-id="${escapeHtml(id)}" tabindex="0" role="link" aria-label="Open ${title}">
      ...
      <div class="book-card-info">
        ...
        <div class="book-card-price-row">
          <div><div class="book-card-price">${formatPrice(book.sale_price || book.salePrice || book.price)}</div>${book.discount > 0 ? `<div class="book-card-old-price">${formatPrice(book.price)}</div>` : ''}</div>
...
if (!document.getElementById('bookora-book-card-premium-styles')) {
  ...
  style.textContent = `
    .book-card-premium{min-width:0;overflow:hidden;background:#fff;border:1px solid var(--border-subtle);border-radius:16px;box-shadow:0 3px 14px rgba(15,23,42,.06);cursor:pointer;}
    .book-card-premium:focus-visible{outline:3px solid rgba(37,99,235,.25);outline-offset:3px;}
    .book-cover-premium{position:relative;width:100%;aspect-ratio:2/3;min-height:0;height:auto;overflow:hidden;background:#172554;}
    .book-cover-image{position:absolute!important;inset:0;width:100%!important;height:100%!important;max-width:none!important;object-fit:cover!important;object-position:center center!important;z-index:1;display:block;transition:transform .35s ease;image-rendering:auto;}
    .book-card-premium:hover .book-cover-image{transform:scale(1.015);}
    ...
    .book-cover-shade{position:absolute;inset:0;z-index:2;background:linear-gradient(to bottom,rgba(2,6,23,.03) 0%,transparent 48%,rgba(2,6,23,.72) 100%);pointer-events:none;}
