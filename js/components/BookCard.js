...
function getCoverUrl(book) {
  const candidates = [book.cover_file_id, book.cover_url, book.cover_image_url, book.coverImageUrl, book.front_cover_url, book.frontCoverUrl, book.cover_image, book.coverImage, book.front_cover, book.frontCover, book.cover, book.thumbnail, book.image_url, book.image, book.thumbnail_url];
  const value = candidates.find(v => typeof v === 'string' && v.trim());
  if (!value) return '';
  const raw = value.trim();

  if (/^[a-zA-Z0-9_-]{20,}$/.test(raw)) return `https://drive.google.com/thumbnail?id=${encodeURIComponent(raw)}&sz=w1000`;
  const idMatch = raw.match(/[?&]id=([a-zA-Z0-9_-]{10,})/i) || raw.match(/\/d\/([a-zA-Z0-9_-]{10,})/i);
  if (/drive\.google\.com/i.test(raw) && idMatch?.[1]) return `https://drive.google.com/thumbnail?id=${encodeURIComponent(idMatch[1])}&sz=w1000`;
  ...
  return `
    <article class="book-card book-card-premium animate-fade-in" data-book-id="${escapeHtml(book.id)}">
      <div class="book-cover-container book-cover-premium" style="background:${fallbackGradient};">
        ${coverUrl ? `<img class="book-cover-image" src="${escapeHtml(coverUrl)}" alt="Cover of ${title}" loading="lazy" decoding="async" onerror="this.style.display='none';this.parentElement.classList.add('cover-image-failed');" />` : ''}
        ...
        <div class="book-cover-content">
          ...
          <div class="book-cover-meta"><span>${author}</span><span class="book-format-pill">${format}</span></div>
        </div>
        ...
        <div class="book-quick-actions"><button class="btn btn-secondary btn-sm quick-preview-btn" data-id="${escapeHtml(book.id)}">Preview</button><a href="#/book/${encodeURIComponent(book.slug || book.id)}" class="btn btn-primary btn-sm">Details →</a></div>
      </div>
      <div class="book-card-info">
        ...
        <div class="book-card-price-row">
          <div><div class="book-card-price">${formatPrice(book.sale_price || book.price)}</div>${book.discount > 0 ? `<div class="book-card-old-price">${formatPrice(book.price)}</div>` : ''}</div>
          ${isInternal ? (hasPurchased ? `<a href="#/library" class="btn btn-secondary btn-sm book-buy-btn">Read</a>` : `<a href="#/checkout/${encodeURIComponent(book.slug || book.id)}" class="btn btn-primary btn-sm book-buy-btn">Buy Now</a>`) : `<a href="${escapeHtml(book.buy_url || book.source_url || '#')}" target="_blank" rel="noopener noreferrer" class="btn btn-external btn-sm book-buy-btn">View &amp; Buy ↗</a>`}
        </div>
      </div>
    </article>
  `;
...
if (!document.getElementById('bookora-book-card-premium-styles')) {
  ...
  style.textContent = `
    ...
    .book-cover-premium{position:relative;width:100%;height:280px;overflow:hidden;background:#172554;}
    ...
    .book-card-title-link{display:block;margin-bottom:.05rem;min-width:0;}
    ...
    .book-card-author{font-size:.74rem;color:var(--text-secondary);margin-bottom:.3rem;}
    .book-card-author span{font-weight:700;color:var(--text-primary);}
    .book-card-rating{min-height:1rem;margin-top:auto;margin-bottom:.4rem;display:flex;align-items:center;}
    .book-card-price-row{border-top:1px solid var(--border-subtle);padding-top:.5rem;display:flex;align-items:center;justify-content:space-between;gap:.5rem;min-width:0;}
    ...
    @media(max-width:700px){.book-cover-premium{height:260px;}.book-card-info{padding:.7rem .75rem;}.book-card-premium .book-quick-actions{opacity:1;transform:none;}}
    @media(max-width:420px){.book-cover-premium{height:250px;}}
  `;
