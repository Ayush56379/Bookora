...
export function renderBookCard(book) {
  ...
  return `<article class="book-card book-card-premium" data-book-id="${esc(id)}" data-book-slug="${slug}" tabindex="0" role="link" aria-label="Open ${title}"><div class="book-cover-container book-cover-premium" style="background:${gradient}">${cover ? `<img class="book-cover-image" src="${esc(cover)}" alt="Cover of ${title}" loading="lazy" fetchpriority="low" decoding="async" onerror="this.style.display='none';this.parentElement.classList.add('cover-image-failed')">` : ''}<div class="book-cover-fallback"><span>${category}</span><strong>${title}</strong>${subtitle ? `<small>${subtitle}</small>` : ''}<small>${author}</small></div><div class="book-cover-shade"></div><div class="book-cover-spine"></div><div class="book-cover-content"><div class="book-cover-topline">${category}</div><h4>${title}</h4>${subtitle ? `<p>${subtitle}</p>` : ''}<div class="book-cover-meta"><span>${author}</span><span class="book-format-pill">${format}</span></div></div><button type="button" class="book-wishlist-btn ${wish ? 'active' : ''}" data-id="${esc(id)}" aria-label="${wish ? 'Remove from Wishlist' : 'Add to Wishlist'}">${wish ? '♥' : '♡'}</button></div><div class="book-card-info"><div class="book-card-meta-row"><span class="badge ${internal ? 'badge-bookora' : 'badge-external'}">${internal ? 'BOOKORA' : 'EXTERNAL'}</span><span class="book-pages">${book.pages ? `${esc(book.pages)} pages` : esc(book.source_domain || 'eBook')}</span></div><a href="#/book/${slug}" class="book-card-title-link"><h3>${title}</h3></a><p class="book-card-author">by <span>${author}</span></p><div class="book-card-rating">${ratingMarkup(book)}</div><div class="book-card-price-row"><div><div class="book-card-price">${formatPrice(sale,currency)}</div>${(discount > 0 || original > sale) ? `<div class="book-card-old-price">${formatPrice(original,currency)}</div>` : ''}</div>${internal ? (purchased ? `<a href="#/library" class="btn btn-secondary btn-sm book-buy-btn">Read</a>` : `<a href="#/checkout/${slug}" class="btn btn-primary btn-sm book-buy-btn">Buy Now</a>`) : `<a href="${esc(book.buy_url || book.source_url || '#')}" target="_blank" rel="noopener noreferrer" class="btn btn-external btn-sm book-buy-btn">View &amp; Buy ↗</a>`}</div></div></article>`;
}
...
if (!window.__BOOKORA_BOOK_CARD_NAV_HANDLER__) {
  ...
  const openCard = card => {
    ...
    const nextHash = `#/book/${target}`;
    if (window.location.hash === nextHash) {
      const app = window.__BOOKORA_APP_INSTANCE__;
      if (app?.route) app.route(true, true);
      return true;
    }
    window.location.hash = nextHash;
    return true;
  };
  // Capture-phase fallback: another component cannot accidentally prevent a
  // book-card tap from changing the SPA hash. Controls/links keep their own
  // native behavior and are deliberately excluded.
  document.addEventListener('click', event => {
    const card = event.target.closest?.('.book-card[data-book-id]');
    if (!card || event.target.closest('button,a,input,select,textarea,[role="button"]')) return;
    openCard(card);
  }, true);
  document.addEventListener('click', event => {
    ...
    // Do not hijack modifier-key/new-tab behavior.
    if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || link.target === '_blank') return;
    // Setting the hash in capture guarantees navigation even if a later
    // non-navigation listener calls preventDefault().
...
if (!window.__BOOKORA_WISHLIST_CARD_HANDLER__) {
  ...
  document.addEventListener('click', async event => {
    const button = event.target.closest('.book-wishlist-btn');
    if (!button) return;
    ...
    try {
      ...
      button.classList.toggle('active', added); button.textContent = added ? '♥' : '♡';
